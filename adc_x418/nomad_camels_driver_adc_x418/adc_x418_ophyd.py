...
class Adc_X418(Device):
    ...
    set_decimal_places_all_channels = Cpt(
        ...
        metadata={
            "units": "",
            "description": "Set number of digits after . shown for all channels, max 4",
        },
    )
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.read_channel_1.read_function = partial(
            self.read_channel_n_read_function, channel_num=1
        )
        self.read_channel_2.read_function = partial(
            self.read_channel_n_read_function, channel_num=2
        )
        self.read_channel_3.read_function = partial(
            self.read_channel_n_read_function, channel_num=3
        )
        self.read_channel_4.read_function = partial(
            self.read_channel_n_read_function, channel_num=4
        )
        self.read_channel_5.read_function = partial(
            self.read_channel_n_read_function, channel_num=5
        )
        self.read_channel_6.read_function = partial(
            self.read_channel_n_read_function, channel_num=6
        )
        self.read_channel_7.read_function = partial(
            self.read_channel_n_read_function, channel_num=7
        )
        self.read_channel_8.read_function = partial(
            self.read_channel_n_read_function, channel_num=8
        )
        self.read_status_channel_1.read_function = partial(
            self.read_status_channel_n_read_function, channel_num=1
        )
        self.read_status_channel_2.read_function = partial(
            self.read_status_channel_n_read_function, channel_num=2
        )
        self.read_status_channel_3.read_function = partial(
            self.read_status_channel_n_read_function, channel_num=3
        )
        self.read_status_channel_4.read_function = partial(
            self.read_status_channel_n_read_function, channel_num=4
        )
        self.read_status_channel_5.read_function = partial(
            self.read_status_channel_n_read_function, channel_num=5
        )
        self.read_status_channel_6.read_function = partial(
            self.read_status_channel_n_read_function, channel_num=6
        )
        self.read_status_channel_7.read_function = partial(
            self.read_status_channel_n_read_function, channel_num=7
        )
        self.read_status_channel_8.read_function = partial(
            self.read_status_channel_n_read_function, channel_num=8
        )
        self.set_range_channel_1.put_function = partial(
            self.set_range_channel_n_put_function, channel_num=1
        )
        self.set_range_channel_2.put_function = partial(
            self.set_range_channel_n_put_function, channel_num=2
        )
        self.set_range_channel_3.put_function = partial(
            self.set_range_channel_n_put_function, channel_num=3
        )
        self.set_range_channel_4.put_function = partial(
            self.set_range_channel_n_put_function, channel_num=4
        )
        self.set_range_channel_5.put_function = partial(
            self.set_range_channel_n_put_function, channel_num=5
        )
        self.set_range_channel_6.put_function = partial(
            self.set_range_channel_n_put_function, channel_num=6
        )
        self.set_range_channel_7.put_function = partial(
            self.set_range_channel_n_put_function, channel_num=7
        )
        self.set_range_channel_8.put_function = partial(
            self.set_range_channel_n_put_function, channel_num=8
        )
        self.set_decimal_places_all_channels.put_function = (
            self.set_decimal_places_all_channels_put_function
        )
        ...
        if self.use_admin_credentials:
            ...
            print(
                "Unable to change settings of ADC without admin credentials. All corresponding commands will be ignored. Use 'Configure Instruments' dialog window to change this."
            )
    ...
    def read_channel_n_read_function(self, channel_num):
        ...
        time.sleep(
            self.sleep_time
        )  ## seems to have random problems from time to time if no waiting after sendall / recv
        try:
            response = self.sock.recv(self.byte_length)
        except:
            err = socket.error.args[0]
            if err == errno.EAGAIN:
                print("eagain")
        ...
        )  ## searching for .json -like part of the response
        if response_match:
    ...
    def read_status_channel_n_read_function(self, channel_num):
        ...
        response_lines = response_data[
            ...
            keepends=False
        )  ## line 0 - names of the fields, line 1 - full of zeroes, lines 2 - 9 correspond to channels 1 - 8
        response_values = response_lines[2 + channel_num].split(
            ","
        )  ## element number 44 describes the range
    ...
    def set_range_channel_n_put_function(self, value, channel_num):
        ...
        message = (
            "GET /io.data?spc0_ioTypeID=9 HTTP/1.1\r\n"
            + "Authorization: Basic "
            + self.encoded_auth_str
            + "\r\n\r\n"
        )
        self.sock.sendall(message.encode())
        time.sleep(self.sleep_time)
        ...
        fields_index = [
            ...
            1,
            2,
        ]  ##replace values for num 44 for range and num 12 for decimal places
        line_to_post = (
            "spc0_settingsTableNum=" + str(channel_num) + "&spc0_settingsTableType=4"
        )
        for i in fields_index:
            if i == 44:
                line_to_post = (
                    ...
                    + "="
                    + value_range
                )  ## copying everything needed except for the range, which is replaced with the new one
            else:
                line_to_post = (
                    line_to_post
                    + "&"
                    ...
                    + response_fields[i]
                    + "="
                    + response_values[i].replace(" ", "+")
                )
        ## sending new status string
        message = (
            "POST /ioUpdate.srv HTTP/1.1\r\n"
            + "Content-Type: application/x-www-form-urlencoded; charset=UTF-8\r\n"
            + "Content-Length: "
            + str(len(line_to_post))
            + "\r\n"
            + "Authorization: Basic "
            + self.encoded_auth_str
            + "\r\n\r\n"
    ...
    def set_decimal_places_all_channels_put_function(self, value=4):
        ...
        for j in range(1, 9):
            ...
            line_to_post = (
                "spc0_settingsTableNum=" + str(j) + "&spc0_settingsTableType=4"
            )
            for i in fields_index:
                if i == 12:
                    line_to_post = (
    ...
    def get_credentials(self):
        ...
        if self.use_admin_credentials:
            ...
            password = simpledialog.askstring(
                "Password", "Enter admin password for ADC X418:", show="*"
            )
        else:
            username = "user"
            password = simpledialog.askstring(
                "Password", "Enter user password for ADC X418:", show="*"
            )
        root_window.destroy()
